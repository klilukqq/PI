class Blowfish {
    ...
    public void generateKey(byte[] key) {
        ...
        int data;
        int j = 0, i;
        //«начение каждого раундового ключа Pn (P1, P2 Е) 
        //складываетс€ по модулю 2 (XOR) с соответст- вующим 
        //элементами исходного ключа
        for (i = 0; i < 18; i++) {
            data = 0x00000000;
            for (int k = 0; k < 4; k++) {
        ...
        //измен€ем элементы матрицы подстановок
        for (i = 0; i < 4; i++) {
            for (j = 0; j < 256; j += 2) {
                encrypt(b);
    ...
    private int F(int x) {
        int a, b, c, d;
        //разбиваем на 4 8-ми битных блока
        d = x & 0xFF;
        x >>= 8; c = x & 0xFF;
        x >>= 8; b = x & 0xFF;
        x >>= 8; a = x & 0xFF;
        //складываем их по модулю 2 в 32, потом по модулю 2(xor), оп€ть сложение
    ...
    void encrypt(byte[] data) {
    	//проводим blocks итераций дл€ шифрование всех данных
        ...
        for (int k = 0, p; k < blocks; k++) {
            ...
            int tmp;
            for (int i = 0; i < 16; i++) {
            	//лева€ с ключом, права€ с F от левой, смена сторон и повтор
                Xl = Xl ^ this.ctx.P[i];
                Xr = F(Xl) ^ Xr;
                tmp = Xl;
                Xl = Xr;
    ...
    void decrypt(byte[] data) {
        ...
        for (int k = 0, p; k < blocks; k++) {
            ...
            int Xr = toWord(data, p + 4);
            int tmp;
            //расшифровка  это просто в обратном пор€дке шифрование
            for (int i = 17; i > 1; i--) {
                Xl = Xl ^ this.ctx.P[i];
                Xr = F(Xl) ^ Xr;
                tmp = Xl;
